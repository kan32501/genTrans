...
class StableVideoDiffusionPipelineControlNeXtReverse(DiffusionPipeline):
    ...
    def __call__(
        ...
        # this step is a semantic embedding of the image to guide the denoising steps to the start & end image
        ...
        # this step creates the tensors that will actually get noised/denoised
        ...
        image1 = image1 + noise_aug_strength * noise # add noise to the image
        image2 = image2 + noise_aug_strength * noise # add noise to the image
        ...
        image1_latent = image1_latent.to(image1_embeddings.dtype) # change data type
        image1_latents = image1_latent.unsqueeze(1).repeat(1, num_frames, 1, 1, 1) # create the N noisy frames

        image2_latent = self._encode_vae_image(image2, device, num_videos_per_prompt, do_classifier_free_guidance)
        image2_latent = image2_latent.to(image2_embeddings.dtype) # change data type
        image2_latents = image2_latent.unsqueeze(1).repeat(1, num_frames, 1, 1, 1) # create the N noisy frames
